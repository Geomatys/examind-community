...
[[_use_embedded_database]]
==== Use an embedded database

When Postgresql subchart is disabled and no external database is specified, then Examind falls back to a deployment using an embedded HSQL file database, stored in Examind home directory (in its persistence mount point).

Example:

[source,yaml]
----
include::../examples/use-embedded-database/values-default-embedded-db.yaml[]
----
...
=== Trust a _specific_ self-signed certificate

To trust a specific self-signed certificate, you must:

. Make target certificate accessible to Examind pod
. Define a custom volume (an empty dir suffice) to hold an overrided version of the java truststore
. Use an init container that:
** mounts the target self-signed/insecure certificate
** mounts the truststore to amend
** import the certificate in the Java truststore override.
. Mount the truststore override into Examind container
. Add a Java system property to import the new truststore

Example:

[source,yaml]
----
include::../examples/access-self-signed-certs/values.yaml[tag=ssc-instructions]
----
