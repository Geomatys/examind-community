 ...
 */
@Component("profile_contact_organization")
public class OrganizationTemplate extends Template {

    public OrganizationTemplate() {
        super(MetadataStandard.ISO_19115, "org/constellation/json/metadata/profile_contact_organization.json");
    }

    @Override
    public String getIdentifier() {
        return "profile_contact_organization";
    }
    ...
    public String getMetadataTitle(Object metadata) {
        if (metadata instanceof ResponsibleParty &&
            ((ResponsibleParty)metadata).getOrganisationName() != null) {
            return ((ResponsibleParty)metadata).getOrganisationName().toString();
        }
        return null;
    }
    ...
    public void setMetadataTitle(String title, Object metadata) {
       if (metadata instanceof DefaultResponsibleParty) {
            ((DefaultResponsibleParty)metadata).setOrganisationName(new SimpleInternationalString(title));
        }
    }
