 ...
 */
package org.constellation.dto.service.config.webdav;
...
public class WebdavContext extends AbstractConfigurationObject {
    ...
    public WebdavContext(final Path rootFile) {
        this.rootFile = rootFile.toAbsolutePath().toUri().toString();
    }

    /**
     * @return the rootFile
     */
    public java.nio.file.Path getRootFile() {
        if (rootFile != null) {
            URI uri = URI.create(rootFile);
            if (uri.getScheme() != null) {
                return Paths.get(uri);
            } else {
                return Paths.get(rootFile);
            }
        }
        return null;
    }
