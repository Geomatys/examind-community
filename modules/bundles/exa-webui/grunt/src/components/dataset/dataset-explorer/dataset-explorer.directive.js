...
function DatasetExplorerController($scope, $rootScope, $element, $timeout, $filter, Examind, DatasetExplorerStyle) {
    ...
    function _select(data, evt, flag) {
        var _data = data;
...
//                        geom.transform("CRS:84", _olView.getProjection());
    ...
    // Listen to event to select/unselect all data
    $scope.$on('select-all-data', function (evt, flag) {
        if (flag) {
            if (self.selected.length > 0) {
                var array = Object.assign([], self.selected);
                array.forEach(function (item) {
                    var find = self.filtered.find(function (elt) {
                        return elt.id === item.id;
                    });
                    if (find) {
                        self.select(find, null, true);
                    }
                });
            }
            self.filtered.forEach(function (item) {
                self.select(item, null, true);
            });
        } else {
            var array = Object.assign([], self.selected);
            array.forEach(function (item) {
