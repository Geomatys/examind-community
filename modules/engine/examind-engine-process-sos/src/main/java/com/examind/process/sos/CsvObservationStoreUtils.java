...
public class CsvObservationStoreUtils {
    ...
    public static SamplingFeature buildFOI(String foiID, final List<DirectPosition> positions, final Set<org.opengis.observation.sampling.SamplingFeature> existingFeatures) {
        ...
        for (org.opengis.observation.sampling.SamplingFeature existingFeature : existingFeatures) {
            if (existingFeature instanceof SamplingFeature &&
                ((SamplingFeature)existingFeature).getGeometry().equals(sp.getGeometry())) {
