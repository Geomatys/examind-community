 ...
 *
 */
public class CsvSplittedObservationStore extends AbstractLineStore {
    
    private final Path stationFile;
    private final Path obsPropFile;
    ...
    @Override
    protected Path getObservationFile() {
        return observationFile;
    }

    @Override
    public Path geProcedureFile() {
        return stationFile;
    }
        ...
        */
        ...
        try (final DataFileReader reader = getDataFileReader(observationFile)) {
            ...
            while (it.hasNext()) {
                ...
                // checks if row matches the observation types (Timeseries, profile, trajectory)
                final List<String> currentMainColumns;
                final ObservationType currentObstType;
                if (typeColumnIndex != -1) {
                    String obsType = asString(line[typeColumnIndex]);
                    if (!obsTypeCodes.contains(obsType)) continue;
                    currentObstType = getObservationTypeFromCode(obsType);
                    
                     // if we set the observation type, we probably set the fill the main column(s) with the correct value
    ...
    public List<ProcedureDataset> getProcedureDatasets(DatasetQuery query) throws DataStoreException {
        ...
        try (final DataFileReader reader = getDataFileReader(observationFile)) {
            ...
            while (it.hasNext()) {
                ...
                final Map<String, Field> fieldMap = buildFieldMaps(currentObstType, sortedMeasureColumns, qualityFields, parameterFields);
