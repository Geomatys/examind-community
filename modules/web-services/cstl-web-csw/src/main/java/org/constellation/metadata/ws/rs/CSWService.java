...
public class CSWService extends OGCWebService<CSWworker> {
        ...
        if (sort != null) {
            ...
            while (tokens.hasMoreTokens()) {
                ...
                if (token.indexOf(':') != -1) {
                    ...
                    sorts.add(FilterXmlFactory.buildSortProperty(version, propName, orderType));
                } else {
                     throw new CstlServiceException("The expression " + token + MALFORMED,
                                                      INVALID_PARAMETER_VALUE, "SortBy");
                }
            }
            sortBy = FilterXmlFactory.buildSortBy(version, sorts);
        }
    ...
    private GetRecordsRequest createNewOpenSearchGetRecordsRequest() throws CstlServiceException {
        ...
        final Query query = CswXmlFactory.createQuery(version, typeNames, setName, null, constraint);
